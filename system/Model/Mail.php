...
class Model_Mail extends Engine_Model
{
				...
				for( $i = 0; $i < count($msg); $i++ )
				{
					if($msg[$i] != null)
					{
						...
						else
						{
							...
							if(!$mailbox["clear"])
							{
								$clear_date = date("YmdHis",
								mktime(date("H",$mailStructure->header->udate),date("i",$mailStructure->header->udate),date("s",$mailStructure->header->udate),date("m",$mailStructure->header->udate),date("d",$mailStructure->header->udate) + $mailbox["clear_days"],
								date("Y",$mailStructure->header->udate)));
	...
	function getMailbox( $email )
	{
		...
		if(count($row) > 0)
		{
			...
			if($row[0]["login"] == "")
			{
				...
				if(($row[0]["login"] == $data["in_login"]) and ($row[0]["password"] == $data["in_password"]))
				{
					...
					if(($row[0]["login"] != $data["in_login"]) or ($row[0]["password"] != $data["in_password"]))
					{
						$data["out_login"] = $row[0]["login"];
						$data["out_password"] = $row[0]["password"];
						$data["out_auth"] = 2;
	...
	function addMailbox( $post )
	{
		...
		$param = array(
			":type" => "in",
			":email" => $post["email"],
			":server" => $post["in_server"],
			":protocol" => $post["in_protocol"],
			":port" => $post["in_port"],
			":login" => $post["in_login"],
			":password" => $post["in_password"],
			":ssl" => $post["in_ssl"],
			":clear" => $clear,
		...
		$param = array(
			":type" => "out",
			":email" => $post["email"],
			":server" => $post["out_server"],
			":protocol" => "SMTP",
			":port" => $post["out_port"],
			":login" => $post["out_login"],
			":password" => $post["out_password"],
			":ssl" => $post["out_ssl"]
		);
		$res->execute($param);
		return $bid;
	...
	function editMailbox( $email, $post )
	{
		...
		$res->execute($param);
		if((!isset($post["out_login"])) or ($post["out_login"] == ""))
		{
			$post["out_login"] = "";
		}
		;
		if((!isset($post["out_password"])) or ($post["out_password"] == ""))
		{
			$post["out_password"] = "";
		}
		;
		...
		for( $j = 0; $j < count($sort); $j++ )
		{
			...
			for( $k = 0; $k < count($ownsort); $k++ )
			{
				if($ownsort[$k]["type"] == "to")
				{
					...
					if($ownsort[$k]["type"] == "from")
					{
						$sql_inc[] = " (`email` = '" . $ownsort[$k]["val"] . "' AND status = 0) ";
					}
					else
						if($ownsort[$k]["type"] == "subject")
						{
